...
export default function PaymentContent() {
  ...
  useEffect(() => {
    const items = searchParams.get('cart');
    if (items) {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        if (data.status === 'completed') {
          ...
          setError(data.details?.reason || 'Payment was cancelled or failed');
          setLoading(false);
  ...
  const clearCart = async () => {
    try {
      ...
      if (!response.ok) {
        console.error('Failed to fetch cart items for clearing.');
        return;
      }
      const currentCartItems: CartItem[] = await response.json();
