...
export default function LoginPage() {
  ...
  return <main className="min-h-screen flex flex-col items-center justify-center px-5 py-12"><Link className="wordmark mb-9" href="/">japhee<span>CAKES & BAKING SCHOOL</span></Link><section className="checkout-panel w-full max-w-md"><p className="eyebrow">A LITTLE SOMETHING FOR YOU</p><h1 className="font-serif text-4xl mb-3">Welcome back.</h1><p className="text-sm text-stone-500 leading-7 mb-7">Sign in to save your favourites, place an order, and explore your next baking course.</p>{error && <p className="notice" role="alert">{error}</p>}<form onSubmit={submit}><label>Email address<input name="email" type="email" autoComplete="email" required /></label><label>Password<input name="password" type="password" autoComplete="current-password" required /></label><button className="bakery-button" disabled={busy}>{busy ? 'Signing in…' : 'Sign in'}</button></form><p className="text-sm mt-7 text-center">New to Japhee? <Link href="/signup" className="text-[#713c46] underline underline-offset-4">Create an account</Link></p></section><Link className="text-link" href="/">← Back to the bakery</Link></main>;
}
