...
export default function Footer({ setActiveTab }: { setActiveTab: (tab: string) => void }) {
  return <footer className="bakery-footer"><div className="bakery-container footer-grid">
    <div><button className="wordmark" onClick={() => setActiveTab('home')}>Japhe&apos;s<span>CAKES & BAKING SCHOOL</span></button><p>Cakes to celebrate.<br />Skills to carry with you.</p></div>
  ...
  </div><div className="bakery-container footer-bottom"><span>© {new Date().getFullYear()} Japhe&apos;s School of Cakes.</span><span>A little sweetness, shared.</span></div></footer>;
}
